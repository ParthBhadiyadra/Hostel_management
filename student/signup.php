...
$temp="";
       ...
       if(0==mysqli_num_rows($res))
        {
            ...
            $n_otp=md5($otp);
            $sql1="INSERT INTO `user`( `email`, `password`, `gender`, `bod`, `fname`, `city`, `mobile`) VALUES('".$email."','".$pass."','".$gen."','".$bod."','".$fname."','".$city."','".$mobile."')";
    ...
    if(isset($_POST['verify']))
    {
        ...
        if(!empty($count))
        {
            ...
            if($row[1]==$user_otp)
            {
              $sql= "UPDATE user set status =1 WHERE  user_id=$pg";
...
<body class=bg>
  ...
  <p class="text-center box" style="font-size: 20px;"> <i class="fas fa-user-secret text-danger"></i> <span> PG / hostel Student Registration
    </span>
  </p>    
  ...
  ?>
        ...
        <form action="" class="shadow-lg p-4 box " method="POST">
          ...
          <div class="form-group">
            <i class="fas fa-user"></i><label for="fname" class="pl-2 font-weight-bold">Full Name</label>
            <input type="text" class="form-control" placeholder="First Name " name="fname" required pattern="[a-zA-Z\s]{2,}" title="only characters are allowed">
          </div>
          <div class="form-group">
            <i class="fas fa-birthday-cake "></i><label for="bod" class="pl-2 font-weight-bold">Birth Date</label>
            <input type="date" class="form-control" placeholder="Birth Date" name="bod" required >
          </div>
          <div class="form-group">
          <i class="fas fa-user"></i><label for="gender" class="pl-2 font-weight-bold">Gender</label>
